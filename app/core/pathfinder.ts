import { Campus, Node, Edge, Path, Instruction, InstructionType, Directions} from "./map-data"
import { Vec3D } from "./vec3d"
import map from "../../mapping/map.json"
...
function FindPath(startId: number, endId: number): Path {
	...
	while (i < path.length - 1) {
		...
		for (let edgeIndex of currentEdges) {
			...
			let currentEdge: Set<number> = new Set<number>([edge.startnode, edge.endnode]);
